...
func (c *ChannelStatus) M3ULine() string {
	headerPieces := []string{
		...
		fmt.Sprintf(`tvc-guide-art="%s"`, c.Rss.Channel.Item.MediaContent.MediaThumbnail[0].URL),
	}
...
func (c *ChannelStatus) XMLTV() xmltv.Channel {
	return xmltv.Channel{
		...
		Icons: []xmltv.Icon{{
			...
			Width:  c.Rss.Channel.Item.MediaContent.Logo.Width,
			Height: c.Rss.Channel.Item.MediaContent.Logo.Height,
		}},
		ID: c.ID,
	}
}
